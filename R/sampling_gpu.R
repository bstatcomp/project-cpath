...
sampling_gpu        <- function (df,
  ...
  if(score2 == "NULL") {
    ...
    mdf1 <- df[ , colnames(df) != "S2"]

    # remove NAs
    mdf1 <- mdf1[!is.na(mdf1$S1), ]

    # rename columns
    colnames(mdf1)[colnames(mdf1) == "S1"] <- "score"
    ...
    mdf_sorted     <- mdf[order(mdf$scoreN, mdf$IDp, mdf$IDs, mdf$time), ]
    pred_df_sorted <- pred_df[order(mdf$scoreN, mdf$IDp, mdf$IDs, mdf$time), ]
    ...
    mdf1 <- df[ , colnames(df) != "S2"]
    mdf2 <- df[ , colnames(df) != "S1"]

    # remove NAs
    mdf1 <- mdf1[!is.na(mdf1$S1), ]
    mdf2 <- mdf2[!is.na(mdf2$S2), ]

    # rename columns
    colnames(mdf1)[colnames(mdf1) == "S1"] <- "score"
    colnames(mdf2)[colnames(mdf2) == "S2"] <- "score"
