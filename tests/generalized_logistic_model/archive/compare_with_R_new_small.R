...
set.seed(1)
in_data <- in_data[in_data$IDs %in% c(1,10), ]
...
RStan_samps3 <- sampling(model_fit, b$data_list, chains = 1, iter = 4000, seed = 1)
saveRDS(RStan_samps3, file = "./tests/generalized_logistic_model/RStan_stan_output_small.rds")
ext <- rstan::extract(RStan_samps3)

library(shinystan)
mss <- as.shinystan(RStan_samps3)
launch_shinystan(mss)
...
saveRDS(b2, file = "./tests/generalized_logistic_model/exe_stan_output_small.rds")
