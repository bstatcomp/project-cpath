...
library(ggplot2)
library(GLMCPath)
source("./tests/generalized_logistic_model/test_scripts/support_scripts/model_wrappers_two_scores.R")

SKIP_MISSING <- F
fo           <- F

# Setup ------------------------------------------------------------------------
datasets <- list.files("./data/generalized_logistic_model/tests/two_scores/", pattern = ".rds", full.names = T)
...
test <- function(k) {
  ...
  if (length(output$model) > 4) { # if we have the R model
    ...
    post_means2 <- colMeans(smp[,grep(pattern = "score2_pred", colnames(smp), fix = T)])
    post_meansA <- c(post_means, post_means2)
    orig_data   <- c(dat$stan_data$score1, dat$stan_data$score2)
    ...
    for (p in 1:dat$stan_data$P2) {
      all_cor <- c(all_cor, cor(post_means2[dat$stan_data$IDp2 == p], 
                                dat$stan_data$score2[dat$stan_data$IDp2 == p]))
    }
    ...
    post_meansA <- jdf$pred[!is.na(jdf$pred)]
  ...
  res <- data.frame(k = k, run_string, model$name, dataset, seed, iter, warmup,
                    ...
                    MedianESS, Q005ESS,
                    TotalCor = cor(post_meansA, orig_data),
                    PPCorMu = mean(all_cor, na.rm = T),
                    PPCorMe = median(all_cor, na.rm = T),
                    PPCor005 = quantile(all_cor, 0.05, na.rm = T),
                    par_tau1_mean = mean(df_smp$tau1),
                    par_tau1_sd = sd(df_smp$tau1),
                    par_tau2_mean = mean(df_smp$tau2),
                    par_tau2_sd = sd(df_smp$tau2),
                    ...
                    par_beta_pbo2_mean = mean(df_smp$beta_pbo2),
                    par_beta_pbo2_sd = sd(df_smp$beta_pbo2))
