...
## rstan -----------------------------------------------------------------------
tmp_data <- in_data[!is.na(in_data$ADAS), ]
...
stan_data <- list(
  ...
  multiplicative_r = 0,
  multiplicative_s = 0,
  X_r              = tmp_data[ ,c("AGE", "SEX")],
  X_s              = as.matrix(tmp_data[ ,c("COMED")]),
  is_pbo           = c(rep(0, M - 4), rep(1, 4)),
  IDp              = reindex(tmp_data$IDp),
  IDs              = reindex(tmp_data$IDs),
  time             = tmp_data$TIME,
  score            = tmp_data$ADAS
)
